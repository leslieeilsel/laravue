...
class LedgerController extends Controller
{
    ...
    //     $data = $data->where('iba_project_schedule.is_audit', 1)
    //             ->whereIn('iba_project_schedule.user_id', $this->seeIds)
    //             ->groupBy('project_id')
    //             ->get()->toArray();
    //     foreach ($data as $k => $row) {
    //         $data[$k]['nature'] = Dict::getOptionsArrByName('建设性质')[$row['build_type']];
    //     }
    ...
    //         ->setCellValue('G7', '年度项目主要建设内容')
    //         ->setCellValue('I7', $data[0]['plan_img_progress']);
    //     $num = 8;
    //     for ($i = 0; $i < count($data); $i++) {
    //         $num = $num + $i * 11;
    //         $spreadsheet->getActiveSheet()->setCellValue('B' . $num, $data[$i]['month'] . '项目进度（需详细说明完成投资额、完成形象进度及相关手续办理情况）');
    //         $spreadsheet->getActiveSheet()->setCellValue('D' . $num, $data[$i]['month_img_progress']);
    //         $spreadsheet->getActiveSheet()->setCellValue('K' . $num, '存在问题（详细描述项目建设中需协调解决的手续办理、征地拆迁及影响项目施工进度的其他问题）');
